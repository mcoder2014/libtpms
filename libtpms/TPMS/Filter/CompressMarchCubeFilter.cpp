#include "CompressMarchCubeFilter.h"

#include <TPMS/MarchBoxUtil.h>
#include <TPMS/MarchBoxTable.h>

CompressMarchCubeFilter::CompressMarchCubeFilter()
    :isoLevel(0)
{

}

Mesh CompressMarchCubeFilter::operator ()(vector<vector<vector<SamplePoint> > > &sampleMatrix)
{
    // 构建采样点为于模型内部缓存
 ...
 * @return
 */
bool CompressMarchCubeFilter::inBoundary(Eigen::Vector3i index, vector<vector<vector<SamplePoint> > > &sampleMatrix)
{
...
void CompressMarchCubeFilter::buildInBoundaryCache(vector<vector<vector<SamplePoint> > > &sampleMatrix)
{
    Vector3i indexBoundary;
...
Mesh::VertexHandle CompressMarchCubeFilter::getVertexHandle(Eigen::Vector3i index, int edgeIndex, vector<vector<vector<SamplePoint> > > &sampleMatrix, Mesh &mesh)
{
...
void CompressMarchCubeFilter::addFaces(int *faces, Eigen::Vector3i index, vector<vector<vector<SamplePoint> > > &sampleMatrix, Mesh &mesh)
{
    while(*faces != -1) {
